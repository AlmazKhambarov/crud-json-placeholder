...
const Posts = () => {
  const { postsData, users, deletedAction, postCommentData, searchedvalue } =
  ...
  const search = postsData?.filter((item) =>
	item.id?.toString()?.toLowerCase().includes(searchedvalue?.toLowerCase())
  );
  useEffect(() => {
    if (!searchedvalue) {
      setSearchedData(paginatedData);
    } else {
